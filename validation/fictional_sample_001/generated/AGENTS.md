...
- Release or public claims: read `agent-guides/release.md`
- Handoff or restart: read `agent-guides/handoff.md`
- Migration or compatibility: read `agent-guides/migration.md`

## Lightweight Guidance Receipt

End every response with `🪶 Core only`, or `🪶 Core + <guides actually read>` using ` · ` in canonical order. Available guide order: `release`, `handoff`, `migration`. This is a declaration, not proof.
